#!/usr/bin/env node
/**
 * BiDi (WebDriver BiDi) Protocol トレース - Chromium版
 *
 * 使用方法:
 *   node examples_nus3/trace-bidi-chromium.mjs
 *
 * このスクリプト1つで：
 * 1. Chromiumを起動
 ...
 * 3. 整形して表示
 *
 * 注意: Chromium は BiDi over CDP を使用します
...
if (isChildProcess) {
  async function runBrowser() {
    console.log('=== Chromium 起動 (BiDi over CDP) ===\n');

    // _bidiChromium を直接使用（内部的にBiDi over CDPを使用）
    const browser = await _bidiChromium.launch({
      headless: false
    });
    ...
    for (const line of lines) {
      ...
      if (sendMatch) {
        try {
          ...
          if (data.method) {
            // BiDi コマンドかどうかチェック（BiDi形式: module.command）
            const isBidi = data.method.includes('.') && 
                          ['session', 'browser', 'browsingContext', 'script', 'network', 'storage', 'permissions', 'emulation'].some(m => data.method.startsWith(m + '.'));
            ...
            if (isBidi) {
              ...
              if (important.includes(data.method)) {
                // メソッド名を表示
                console.log(`\n📤 [BiDi ${String(commandCounter).padStart(3)}] ${data.method}`);
              ...
              cdpDomains[domain] = (cdpDomains[domain] || 0) + 1;
            }
          }
        } catch (e) {
          // パースエラーは無視
  ...
  child.on('close', (code) => {
    ...
    console.log(`Total CDP commands (internal): ${cdpCounter}`);
    ...
    if (Object.keys(cdpDomains).length > 0) {
      console.log('\nCDP domains (内部変換):');
      Object.entries(cdpDomains)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 5)
    ...
    console.log('   Chromium では BiDi over CDP として実装されています');
    console.log('   - BiDi コマンド → chromium-bidi → CDP コマンドに変換');
    console.log('   - chromium-bidi: https://github.com/GoogleChromeLabs/chromium-bidi');
